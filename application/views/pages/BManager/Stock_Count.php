...
<div class = "row">
    <!-- <button class="btn btn-success" onclick="add_person()" title="Add new Administrator"><i class="glyphicon glyphicon-plus"></i> Add New Stock</button> -->
    <br />
    <br />
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?php echo site_url('Stock_Count/Stock_list')?>",
                "type": "POST"
            },
    ...
    function add_person()
    {
        ...
        $('.modal-title').text('Add New Stock'); // Set Title to Bootstrap modal title
    }

    function edit_person(stock_id)
    {
        save_method = 'update';
        $('#form')[0].reset(); // reset form on modals
        ...
        $.ajax({
            url : "<?php echo site_url('Stock_Count/Stock_edit/')?>/" + stock_id,
            type: "GET",
            dataType: "JSON",
    ...
    function save()
    {
        ...
        if(save_method == 'add')
        {
            url = "<?php echo site_url('Stock_Count/Stock_add')?>";
        }
        else
        {
            url = "<?php echo site_url('Stock_Count/Stock_update')?>";
        }
    ...
    function delete_person(stock_id)
    {
        ...
        swal({
            ...
            if (isConfirm) {
                ...
                $.ajax({
                    url : "<?php echo site_url('Stock_Count/Stock_delete')?>/"+stock_id,
                    type: "POST",
                    dataType: "JSON",
                    success: function(data)
                    {
    ...
    function view_person(stock_id)
    {
        $.ajax({
            url : "<?php echo site_url('Stock_Count/Stock_list_by_id')?>/" + stock_id,
            type: "GET",
